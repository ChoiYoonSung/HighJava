...
/*
	...
	ReentrantLock : Read 및 Write 구분없이 사용하기 위핸 락 클래스, 동기화 처리를 위해 사용됨.
...
class LockAccount{
	...
	public boolean withdraw(int money) {
		...
		try {
			if(balance >= money) {
				for (int i = 1; i < 1000000000; i++) {} // 시간 벌기용
				balance -= money;
			...
			lock.unlock();	// 락 해제 
							// 무조건적으로 예외가 발생하게 되는데 락을 반납해주지 않는다면 
							// 계속하여 동기화 상태가 지속되어 치명적인 오류가 발생할 수도 있다
		}
		return chk;
	}
