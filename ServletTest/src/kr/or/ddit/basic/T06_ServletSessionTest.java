...
/**
	세선(HttpSession) 객체에 대하여
	
	- 세션을 통해서 사용자(웹브라우저)별로 구분하여 정보를 관리할 수 있다. (세션ID 이용)
	    쿠키를 사용할 때 보다 보안이 향상된다.(정보가 서버에 저장되기 떄문)
	
	- 세션객체를 가져오는 방법
	HttpSession session = request.getSession(boolean값); default로 true를 가진다.
	boolean값 : true인 경우 => 세션객체가 존재하지 않으면 새로 생성함 (있으면 기존 세션, 없으면 새로운 세션 생성)
			   false인 경우 => 세션객체가 존재하지 않으면 null 리턴함 (있으면 기존 세션, 없으면 null 리턴)
	
	- 세션 삭제처리 방법
	1. invalidate()메서드 호출 => 세션 자체를 삭제하는 메서드
