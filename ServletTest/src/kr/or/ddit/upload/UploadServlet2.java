...
public class UploadServlet2 extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		if(ServletFileUpload.isMultipartContent(req)) {
			// 인코딩 타입이 moltipart/form-data 인 경우
				...
				if(formItems != null && formItems.size() > 0) {
					for(FileItem item : formItems) {
						if(!item.isFormField()) {// 파일인 경우
							...
							req.setAttribute("message", "업로드 완료  => 파일명 : " + fileName);
						}else {	// 폼 데이터인 경우
							// 폼필드의 값이 한글인 경우에는 해당 문자열을 적절히 변환해주어야 한다.
							// 1. value = new String(fileItem.getString().getBytes("8859-1"), "UTF-8");
							// 2. value = fileItem.getString("UTF-8");
