...
public class T16_NonSerializableParentTest {
	/**
	 	부모 클래스가 Serializable 인터페이스를 구현하고 싶지 않을 경우
	 	부모 객체의 필드값 처리 방법
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		// 부모클래스에 Serializable을 해주게 된다면 자식클래스도 모두 Serializable된다.
		// 설계상으로 좋은 설계가 아님
	}
}
...
class Child extends Parent implements Serializable{
	...
	/**
	 * 직렬화 할때 자동으로 호출
	 * (접근 제어자가 private이 아니면 자동호출되지 않는다.)
	 * @param oos
	 * @throws IOException 
	 */
	private void writeObject(ObjectOutputStream out)
		...
		out.defaultWriteObject();	// default작업 실행
