...
public class WebController extends HttpServlet{
	...
	private Map<String, CommandHandler> cmmHandlerMap 
			= new HashMap<>(); // 매핑정보 저장 (핸들러 객체 저장용 맵)
	...
	public void init(ServletConfig config) throws ServletException {
		...
		String configFileRealPath = config.getServletContext()
	...
	private void process(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//---------------------------
		// 커맨드 형식 (수정 필요 없음)
		// 기존 소스를 수정하는 것 보다 클래스가 늘어나는 것이 부담되지 않는다.
		...
		// VIEW 화면 처리
		if(viewPage != null) {	// 뷰페이지 존재시
			if(handler.isRedirect(req)) {	// 리다이렉트 필요한 경우
		...
		/*
		 	...
		 	Invoker : 사용자 요청에 대응되는 적당한 커멘드 객체를 찾아 실행해주는 역할을 하는 객체
		 	
		 	- 장점 : 요청을 처리하는 객체(Invoker)로부터 실제 수행기능을 분리함으로써,
