...
public class MultiChatServer {
	...
	public void startServer() {
		...
		//	try with resource -> 이 문법을 이용하여 생성하게 되면
		//	try 이후 자동으로 close 해준다.
	...
	//	=> Inner클래스에서는 부모 클래스의 멤버들을 직접 사용할 수있다.
	class ServerReceiver extends Thread{
		...
		public void run() {
			try {
				//서버에서는 클라이언트가 보내느 최초의 메시지, 즉 대화명을 수신해야 한다.
				name = dis.readUTF();
				...
				// 이 finally영역이 실행된다는 것은 클라이언트의 접속이 종료되었음을 의미한다.
				System.out.println(name + "님이 나갔습니다.");
				//Map에서 해당 대화명을 삭제한다.
				clients.remove(name);
